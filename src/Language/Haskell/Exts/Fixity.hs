...
module Language.Haskell.Exts.Fixity
    ...
    , AppFixity(..), applyFixities
    , FixityError(..), printFixityError, fixityErrorPos
    ) where
...
import Control.Monad.Writer hiding (Alt, fix)
...
data FixityError = FixityError SrcLoc Fixity Fixity

fixityErrorPos :: FixityError -> SrcLoc
fixityErrorPos (FixityError ss _ _) = ss

printFixityError :: FixityError -> String
printFixityError (FixityError _ (Fixity lr1 n op1) (Fixity lr2  _ op2))
  = concat $ intersperse " "
      [prettyPrint op1 ++ printAssoc lr1
      , "and"
      , prettyPrint op2 ++ printAssoc lr2
      , "have precedence", show n]
  where
    printAssoc :: Assoc () -> String
    printAssoc (AssocNone {}) = "{}"
    printAssoc (AssocLeft {}) = "{l}"
    printAssoc (AssocRight {}) = "{r}"


applyFixities :: AppFixity ast
              => [Fixity]
              -> ast SrcSpanInfo
              -> Either [FixityError] (ast SrcSpanInfo)
applyFixities fixs ast =
  case runWriter $ applyFixitiesInternal fixs ast of
    (_, es@(_:_)) -> Left es
    (a, [])       -> return a
...
class AppFixity ast where
  ...
  applyFixitiesInternal :: MonadWriter [FixityError] m
...
instance AppFixity Pat where
  applyFixitiesInternal fixs' tope = infFix fixs' <=< leafFixP fixs' $ tope
    where -- This is the real meat case. We can assume a left-associative list to begin with.
...
-- Same using patterns
askFixityP :: [Fixity] -> QName l -> Fixity
askFixityP xs qn = askFix xs (g (() <$ qn))
    where
        g (Special _ (Cons _)) = UnQual () (Symbol () ":")
        g x                  = x

askFix :: [Fixity] -> QName l -> Fixity
askFix xs v = lookupWithDefault (defaultFixity v) (() <$ v) mp
    where
        lookupWithDefault def k mp' = fromMaybe def $ lookup k mp'

        mp = [(x,f) | f@(Fixity _ _ x) <- xs]

defaultFixity :: QName l -> Fixity
defaultFixity v = Fixity assocLeft 9 (() <$ v)

-- | Built-in fixity for prefix minus
prefixMinusFixity :: Fixity
prefixMinusFixity = Fixity assocLeft 6 (UnQual () (Ident () "-"))
...
instance AppFixity Module where
    applyFixitiesInternal fixs (Module l mmh prs imp decls) =
        liftM (Module l mmh prs imp) $ appFixDecls mmn fixs decls
      where mmn = getMmn mmh
            getMmn (Just (ModuleHead _ n _ _)) = Just n
            getMmn _ = Nothing
    applyFixitiesInternal fixs (XmlPage l mn os xn xas mexp cs) =
        liftM3 (XmlPage l mn os xn) (fix xas) (fix mexp) (fix cs)
      where fix xs = mapM (applyFixitiesInternal fixs) xs
    applyFixitiesInternal fixs (XmlHybrid l mmh prs imp decls xn xas mexp cs) =
      ...
      where mmn = getMmn mmh
            ...
            fixe xs = let extraFixs = getFixities mmn decls
                       in mapM (applyFixitiesInternal (fixs++extraFixs)) xs

instance AppFixity Decl where
    applyFixitiesInternal fixs decl = case decl of
        ...
        PatBind l p rhs bs      ->
         let extraFix x = applyFixitiesInternal (fixs ++ maybe [] getBindFixities bs) x
...
appFixDecls :: MonadWriter [FixityError]  m
...
appFixDecls mmdl fixs decls =
    let extraFixs = getFixities mmdl decls
     in mapM (applyFixitiesInternal (fixs++extraFixs)) decls
...
instance AppFixity Annotation where
    applyFixitiesInternal fixs ann' = case ann' of
...
instance AppFixity ClassDecl where
    applyFixitiesInternal fixs (ClsDecl l decl) = liftM (ClsDecl l) $ applyFixitiesInternal fixs decl
    applyFixitiesInternal _ cdecl = return cdecl

instance AppFixity InstDecl where
    applyFixitiesInternal fixs (InsDecl l decl) = liftM (InsDecl l) $ applyFixitiesInternal fixs decl
    applyFixitiesInternal _ idecl = return idecl

instance AppFixity Match where
    applyFixitiesInternal fixs match = case match of
      ...
      where fix bs x = applyFixitiesInternal fixs' x
             where fixs' = fixs ++ maybe [] getBindFixities bs

instance AppFixity Rhs where
    applyFixitiesInternal fixs rhs = case rhs of
...
instance AppFixity GuardedRhs where
    applyFixitiesInternal fixs (GuardedRhs l stmts e) = liftM2 (GuardedRhs l) (mapM fix stmts) $ fix e
      where fix x = applyFixitiesInternal fixs x

instance AppFixity PatField where
    applyFixitiesInternal fixs (PFieldPat l n p) = liftM (PFieldPat l n) $ applyFixitiesInternal fixs p
    applyFixitiesInternal _ pf = return pf

instance AppFixity RPat where
    applyFixitiesInternal fixs rp' = case rp' of
...
instance AppFixity PXAttr where
    applyFixitiesInternal fixs (PXAttr l n p) = liftM (PXAttr l n) $ applyFixitiesInternal fixs p

instance AppFixity Stmt where
    applyFixitiesInternal fixs stmt = case stmt of
...
instance AppFixity Binds where
    applyFixitiesInternal fixs bs = case bs of
...
instance AppFixity IPBind where
    applyFixitiesInternal fixs (IPBind l n e) = liftM (IPBind l n) $ applyFixitiesInternal fixs e

instance AppFixity FieldUpdate where
    applyFixitiesInternal fixs (FieldUpdate l n e) = liftM (FieldUpdate l n) $ applyFixitiesInternal fixs e
    applyFixitiesInternal _ fup = return fup

instance AppFixity Alt where
    applyFixitiesInternal fixs (Alt l p galts bs) = liftM3 (Alt l) (fix p) (fix galts) (mapM fix bs)
      where fix x = applyFixitiesInternal fixs x

instance AppFixity QualStmt where
    applyFixitiesInternal fixs qstmt = case qstmt of
...
instance AppFixity Bracket where
    applyFixitiesInternal fixs br = case br of
...
instance AppFixity Splice where
    applyFixitiesInternal fixs (ParenSplice l e) = liftM (ParenSplice l) $ applyFixitiesInternal fixs e
    applyFixitiesInternal _ s = return s

instance AppFixity XAttr where
    applyFixitiesInternal fixs (XAttr l n e) = liftM (XAttr l n) $ applyFixitiesInternal fixs e
...
leafFix :: MonadWriter [FixityError] m => [Fixity] -> Exp SrcSpanInfo -> m (Exp SrcSpanInfo)
leafFix fixs e' = case e' of
    ...
    Let l bs e                ->
        let extraFix x = applyFixitiesInternal (fixs ++ getBindFixities bs) x
    ...
    _                         -> return e'
  where
    fix x = applyFixitiesInternal fixs x

leafFixP :: MonadWriter [FixityError] m
